...
# ---------------------------------------------------------------------------
# O29 sibling wall — genealogy only, never an activity's other outputs.
# ---------------------------------------------------------------------------


async def test_siblings_generated_by_the_same_activity_are_excluded() -> None:
    # 50 sibling entities generated by the same activity as the origin. A
    # bidirectional walk would pull all of them in via the activity's object
    # role (`?other prov:wasGeneratedBy ?activity`); O29 forbids that.
    siblings_ttl = "\n".join(
        f'ex:sibling-{i} a prov:Entity ; prov:wasGeneratedBy ex:activity-aggregate .'
        for i in range(50)
    )
    client = _pyoxi_client({WEATHERLOG_GRAPH: _WEATHERLOG_TTL + "\n" + siblings_ttl})
    out = await prov_graph(client, DIGEST)
    ids = {n["id"] for n in out["graph"]["nodes"]}
    assert ids == {
        DIGEST,
...
async def test_downstream_one_hop_only_not_expanded_further() -> None:
    # A node that references the origin via wasDerivedFrom is admitted (one
    # downstream hop from origin), but its own upstream lineage must not be
    # walked — that would re-open the sibling wall from the other side.
    downstream_entity = EX_W + "downstream-report"
    downstream_activity = EX_W + "activity-downstream"
    ttl = _WEATHERLOG_TTL + (
        f"\n<{downstream_entity}> a prov:Entity ;"
        f"    prov:wasDerivedFrom <{DIGEST}> ;"
        f"    prov:wasGeneratedBy <{downstream_activity}> .\n"
        f"<{downstream_activity}> a prov:Activity .\n"
    )
    client = _pyoxi_client({WEATHERLOG_GRAPH: ttl})
    out = await prov_graph(client, DIGEST)
    ids = {n["id"] for n in out["graph"]["nodes"]}
    assert downstream_entity in ids
    assert (DIGEST, downstream_entity, "derived") in _edge_set(out)
    assert downstream_activity not in ids
