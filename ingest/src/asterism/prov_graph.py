...
def _edge_step_query(from_clause: str, frontier: list[str]) -> str:
    """One BFS step: every PROV-triple touching a ``frontier`` node, as subject
    or object, scoped per version graph (``GRAPH ?g``) so the caller can attach
    ``dataset_id``/``snapshot`` to whatever it discovers there.
    ...
    return (
        ...
        f"  VALUES ?n {{ {values} }}\n"
        "  GRAPH ?g {\n"
        "    { ?n ?pred ?o . BIND(?n AS ?s) }\n"
        "    UNION\n"
        "    { ?s ?pred ?n . BIND(?n AS ?o) }\n"
        "  }\n"
        "}\n"
        "ORDER BY ?s ?pred ?o\n"
    )
...
async def prov_graph(
    ...
    """Follow only the 7 PROV-O predicates (both directions) out from ``iri``
    and return a renderer-ready directed graph: ``{iri, found, graph, materials}``.
    ...
    ordered chain (the caller lays it out). BFS is bounded by ``max_depth``
    (edge-discovery round trips) and ``max_nodes`` (total node count); either
    limit cutting the walk short sets ``graph.truncated``.
    ...
    while frontier:
        ...
        if len(node_graph) >= max_nodes:
            ...
            break
        depth += 1
        rows = await _run_select(client, _edge_step_query(named_from, frontier))
        next_frontier: list[str] = []
        for row in rows:
            s, pred, o, g = (_cell(row, k) for k in ("s", "pred", "o", "g"))
            if s is None or o is None or pred is None:
                continue
            label = _EDGE_LABELS.get(pred)
            if label is None:
                continue
            new_nodes = [n for n in (s, o) if n not in node_graph]
            if new_nodes and len(node_graph) + len(new_nodes) > max_nodes:
                truncated = True
                continue
            edge_key = (o, s, label)
            if edge_key not in edge_seen:
                edge_seen.add(edge_key)
                edges.append({"from": o, "to": s, "label": label})
            for node in (s, o):
                if node not in node_graph:
                    node_graph[node] = g
                    next_frontier.append(node)
                elif node_graph[node] is None and g:
                    node_graph[node] = g
