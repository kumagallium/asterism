...
def dialect_rows(src: Path | str, dialect: SourceDialect) -> Iterator[list[str]]:
    ...
    The single definition of the normalization read rules (ADR §runtime):
    decode with ``dialect.encoding`` **strict** (a decode error is a real error,
    not something to paper over; CRLF is handled by text-mode decoding), drop
    ``skip_rows`` lines and blank lines, then split — ``whitespace`` splits on
    runs of spaces/tabs and drops empty tokens (collapse is implied); a
    single-char delimiter parses via :mod:`csv` (so a quoted delimiter does not
    split a cell), dropping empty tokens when ``collapse``. Lazy (line-streamed),
    so reading just the header row does not slurp a large export.
    """
    with Path(src).open("r", encoding=dialect.encoding, errors="strict", newline=None) as fh:
        lines = (
            line.rstrip("\n")
            for i, line in enumerate(fh)
            if i >= dialect.skip_rows and line.strip()
        )
        if dialect.delimiter == WHITESPACE:
            for ln in lines:
                yield [tok for tok in _WHITESPACE_RUN.split(ln) if tok]
        else:
            for row in csv.reader(lines, delimiter=dialect.delimiter):
                yield [tok for tok in row if tok] if dialect.collapse else row
...
def normalize_source(src: Path | str, dialect: SourceDialect, dest: Path | str) -> Path:
    ...
    Memory-bounded: rows stream from :func:`dialect_rows` into :func:`csv.writer`.
    """
    dest_path = Path(dest)
    with dest_path.open("w", encoding="utf-8", newline="") as out:
        csv.writer(out).writerows(dialect_rows(src, dialect))
    return dest_path
