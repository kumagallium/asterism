...
# e.g. "Fri Sep 01 2017 18:19:39 GMT+0900 (Japan Standard Time)". We keep the
# raw string on the entity (dcterms:created) for fidelity, but PROV's
# prov:atTime wants an xsd:dateTime, so this parses best-effort to ISO 8601.
_TZ_NAME_SUFFIX = re.compile(r"\s*\([^)]*\)\s*$")  # " (Japan Standard Time)"


def parse_curator_timestamp(raw: str) -> str | None:
    """Parse a JS Date.toString() timestamp to ISO 8601, or None on failure."""
    if not raw:
        return None
