...
# 画面・欄の名前も UI の実文言でなければならない。ボタン/タブ/メニューだけを見ていた
# ときに、実在しない画面名（「ID を確かめる」— 正しくは「ID のつけかた」の②）が
# マニュアルに入り、検査を素通りした。文型を「…」＋画面/欄 に絞ってあるのは、
# 比喩や例示の引用（「わからないこと」「この試料のピークを全部」…）まで拾わないため。
