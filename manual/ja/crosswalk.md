...
データを取り込むときの「つながりを選ぶ」画面で ☑ を付けた列どうしは、値が
重なっていれば公開したときに自動で結ばれます。ここで手作業は不要です —
結果はこの画面の一覧に「☑ から自動で作られました」と表示され、確かめたり
外したりできます。
...
- 一致した値の実例（実際のデータの値）
- つながるデータセットと、その項目（「種類 › 項目名」の形。例:
  `Composition › 試料化学組成`。同じ名前の項目を複数の種類が持っていても、
  どの種類の値でつなぐかが分かります）
...
- 「このつながりを削除」ボタン（確認のうえで、つながりそのもの＝結合のグラ
  フと設定を消します。元のデータセットには触れないので、同じ設定でいつでも
  作り直せます）
...
## つながったものは 1 つのページで見られる

☑ を付けた列どうしが自動で結ばれると（上記「つながりとは」参照）、その値
が指す対象自体を、1 つのページとして開けるようになります。

- 別々のデータセットから来た記録のページを開くと、見出しの下に「別の
  データセットにも同じものがあります → まとめて見る」という帯が出ます。
  これは、いま見ているものが他のデータセットにもつながっていることを
  知らせるものです。帯が出ないときは、そのものについて他のデータセット
  とのつながりは見つかっていません。
- 帯の「まとめて見る」を押すと、つながった対象そのもののページに移りま
  す。このページには、つながっている両方（またはそれ以上）のデータセッ
  トにある、そのものに関する記録が一覧されます。
- このページからも、通常の 1 件のページと同じように観点（集計・並べ替え
  などの見方）を作れます。つまり、片方のデータセットだけでなく、つながっ
  た両方のデータセットの記録をまとめて使った観点を、AI と会話しながら作
  れます。
