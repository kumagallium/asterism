...
# はじめる — かんたんモードの 7 ステップ <Badge type="tip" text="v0.9.0 (2026-07-23) で追加" />
...
AI が意味を決められなかった列は「まだ取り込んでいない項目」としてまとまって
表示されます。行が 2 つ以上あるときは、その表の上の「すべて取り込む」ボタン・
「すべて取り込まない」ボタン・「空の意味に元の列名を使う」ボタンでまとめて
決められます <Badge type="tip" text="v0.19.0 (2026-08-25) で追加" />。

確かめたら「この意味で進む」ボタンで次へ。読み取りの設定に戻りたいときは
「読み方の設定に戻る」ボタンです。

![意味をつける画面。列ごとに意味・単位・取り込むかを確かめる](../screenshots/kantan-meanings.png)

## 4. つながりを選ぶ <Badge type="tip" text="v0.31.0 (2026-08-31) で追加" />
...
## 5. かたちをたしかめる <Badge type="tip" text="v0.31.0 (2026-08-31) で追加" />
...
前書きの無い、表だけのファイルでは「カード」の箱はありません — ファイル全体
を表す値がそもそも無いためで、正常な形です。レコードと受け口だけの図になり、
受け口への参照は各レコードから引かれます。
...
形が違うときは、「形が違うとき（直す）」の下の 4 つから直しかたを選びます。
...
- 「AI にもう一度考えさせる」ボタン — いまの設計を渡して、注文を付けた
  ところだけ AI に直させます。この画面でやった編集（種類の名前・ID の作り方・
  削除・切り出し）は残ります。データセットの名前（ID に使われる英字名）は
  「名前・ID を直す」の中で変えられます
...
ボタンで相談チャットが開きます。前の画面に戻りたいときは「戻ってやり直す」
ボタンです。

続いて「数の確認」の画面が出ます。元のファイルの行数と、できた記録の件数が
合っているかを見てください。件数が行数よりずっと少ないときは、複数の行が
1 件にまとまっています。意図と違えば「⑤ かたちをたしかめる に戻る」ボタンで戻れます。
...
あれば「何かおかしい — ③ 意味をつける に戻る」ボタンで 3. の画面に戻れます。
「⑤ かたちをたしかめる に戻る」ボタンで 5. に戻ることもできます。
...
## もっと詳しく操作したいとき <Badge type="tip" text="v0.8.0 (2026-07-22) で追加" />
