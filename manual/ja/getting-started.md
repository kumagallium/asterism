...
迷ったときは、この一覧のすぐ上にある「つながりを AI に相談」ボタン・
「分け方を AI に相談」ボタンで相談チャットが開きます（文面が入った状態で
開くだけなので、送るかどうかはあなたが決めます）。

わからなければ、いったん「ファイル全体」のままで進んで構いません。公開する
までは、この画面に戻って何度でも直せます（種類を足す・消す・項目を移す）。
動かせなくなるのは、公開して ID を配ったあとだけです。

1 行に別々のもの（例: 測定の記録と、測定した対象）が混ざっているときは、
「＋ 同じ ID で種類を足す」ボタンで、いまの ID をそのまま使う種類をもう 1 つ
足して項目を分けられます。名前は②と同じ規則で、英字ではじまる英数字で書き
ます。「足す」ボタンを押すと種類が増えるので、その種類に載せる項目は右の
「載せる種類」で移してください。

逆に、AI の案が行ごとの種類だけで「ファイル全体で 1 件」が無いときは、⚠ と
「ファイル全体の種類を作る」ボタンが出ます。押すと、ファイルに一度だけ書かれて
いる値（カタログ番号や測定条件など）が、まとまった 1 件に分かれます。

② 「ID がひとつずつを指しているか確かめる」では、①で決めた種類ごとに、
名前と ID の作り方を確かめます。⚠ が付いた種類だけ見れば足ります。

同じ ID で数える種類が 2 つあり、**しかも同じ項目を持っている**ときは、②の
見出しの下に ⚠ が出ます。重複そのものが禁止なわけではありません。出口は
2 つです — 同じものの写しなら要らない方を消す、別のものなら「載せる種類」で
項目を分ける（分ければ ⚠ は消えます）。迷ったら ⚠ の下の「重複を AI に相談」
ボタンで、どちらの種類をどう扱うべきかを具体的に聞けます。なお、名前が違う
だけのまったく同じ受け口を AI の案が 2 つ出したときは、画面に出す前に自動で
1 つに畳まれます。
