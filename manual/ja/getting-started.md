...
同じ番号が 2 つのものを指しているとき（1 枚の記録と、その記録が説明している
対象など）は、その下の「＋ 同じ ID で種類を足す」ボタンで、自分で種類を足して
項目を分けられます。名前は②と同じ規則で、英字ではじまる英数字で書きます。
「足す」ボタンを押すと種類が増えるので、その種類に載せる項目は右の
「載せる種類」で移してください。
...
種類を二度書いた形なので ⚠ が出ます。重複そのものが禁止なわけではありません。
出口は 2 つです — 同じものの写しなら要らない方を消す、別のものなら「載せる種類」
で項目を分ける（分ければ ⚠ は消えます）。迷ったら「重複を AI に相談」ボタンで、
どちらの種類をどう扱うべきかを具体的に聞けます。
