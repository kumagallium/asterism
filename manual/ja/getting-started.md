...
「ID のつけかた」の画面では、どの値に ID を付けるかを決めます。ID の付いた
値は、あとで別のファイルや外のデータと「同じ 1 件」としてつながり、引用にも
使えます。画面は 2 つに分かれています。
...
各列が何を意味するか（例:「Ic」は臨界電流、単位は A）を AI が調べて
下書きに取り込みます。取り込み後の表で、列ごとの意味・単位を確認・修正
できます。ID の決めかたをやり直したいときは「ID のつけかたに戻る」
ボタンで 3. の画面に戻れます。
