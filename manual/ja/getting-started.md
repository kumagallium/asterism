...
「（行ごとに変わる）」と表示されます。ファイル単位の値に 2 つ以上 ☑ を
...
図の下に ⚠ が出ます。そのままでも進めますが、違うときは「ID を候補から
選び直す」で直せます。また、同じ ID で数える種類が 2 つあり、しかも同じ
...
- 「ID を候補から選び直す」ボタン — 種類ごとの ID の作り方を確かめて、候補
  から選び直せます。仮の ID を直すときの入口です
