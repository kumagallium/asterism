...
| 取り込み（ingest）・**未公開** | 触らない（保存時のものを使う） | 書く | 公開前でもカタログに説明が要る |
| 取り込み（ingest）・**公開済みの見直し** | 触らない | 書かない | 上と同じ理由。街の説明は公開の瞬間に切り替わる。「公開済みか」は job の末尾で**読み直す**（長い取り込みの最中に promote が挟まる競合を塞ぐ） |
| 追記（append） | 触らない | 触らない | 説明に件数を持たないので、足しても古くならない（§3） |
| 公開（promote） | 触らない | **必ず書き直す** | 街に出た説明はここで確定する（`published_subjects` と同じ流儀）。`metadata.ttl` が空なら**落とす**（消えた説明を配り続けない） |
