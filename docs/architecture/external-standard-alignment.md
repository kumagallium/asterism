...
## 11. 設計の段で「標準の綴り」を渡す (2026-08-29)

### Trigger
利用者評価: 「クラス名はすでにある共通の語彙から選ぶほうが良かったりしますか？
それなら共通の語彙を候補で指すべき？」→ 実際に画面で見たあと「名前だけで同じかどうかを
その場で判断するのは厳しいので、今のままで良いかも。ただ、**AI の候補が自動的に
寄せ気味になっている**と良さそう？」

つまり求められているのは、**人に語彙を選ばせること**ではなく（それは §8 の接地の段の
仕事で、判断を増やす）、**AI が最初から標準どおりの言葉で名づけること**。

### 実測した穴
骨格プロンプトには以前から「標準語彙を再利用せよ」と書いてあったが、**どの語彙にどんな
語があるのかは一切渡していなかった**。モデルは記憶から綴りを当てるしかなく、実 XRD カードで
出てきた名前は `Crystal`。カタログにあるのは `CrystalStructure` で、この 1 文字違いのために
接地の段で「Crystal と CrystalStructure は同じものか？」という**人にしか答えられない問い**が
発生する。綴りが一致していれば同じ画面が「これで良いか」の 1 クリックになる。

### Decision
`catalog_terms(kind="class")` の**綴りだけ**（45 件・1.3KB）を骨格プロンプトに載せる。

| 渡すもの | 渡さないもの | なぜ |
|---|---|---|
| 語（`CrystalStructure`） | CURIE（`cmso:CrystalStructure`） | CURIE を渡すとモデルはそれを設計に**書ける**。書いた時点で「このクラスは cmso のそれと同一だ」という誰も検めていない主張が設計に入る |
| 「同じ意味ならこの綴りで、自分の接頭辞の下に作れ」 | 「この語彙を使え」 | 綴りの薦めであって同一性の主張ではない。同一性を確かめるのは §8 の接地の段（人が押す） |
| 「当てはまるものが無ければデータの言葉のまま」 | — | 無理に寄せると K22（機械が意味を発明しない）を破る |

closed set なので**存在しない語彙語を薦めることは原理的にできない**。カタログは `ingest`
パッケージにあり、step0 単体で入れたときは節ごと出ない（名づけはデータの言葉に落ちる）。

### 効果の測り方
この変更が効いたかどうかは**接地の段で exact 一致が何件出るか**で測る。実モデルでの
計測は未実施（スタブ LLM は決め打ちを返すため、確かめたのは「節を足しても guided
decoding が通り骨格が返ること」まで）。
