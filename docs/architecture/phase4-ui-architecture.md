...
- **Decision**: API は `subprocess` で `asterism-propose` を叩くのではなく、`asterism_step0.propose` 等を **import して関数呼び出し**する。
...
| **Mapping (束縛)** | `ingest/src/asterism/starrydata.py` (CSV→triples) + `data/togomcp/mie/<name>.yaml` (探索メタ + ShEx) | この CSV をどう語彙に結びつけるか。**速く変わる・dataset/目的ごと・ローカル** |
...
- **Decision**: Ask の回答生成は **asterism core API に入れない**。回答を組み立てるのは core の外の **消費層 (demo-agent)** で、UI はその 2 つの HTTP 契約だけを叩いて描画する。core の typed query tools は Claude 非依存のまま保つ (ADR §1/§5)。
