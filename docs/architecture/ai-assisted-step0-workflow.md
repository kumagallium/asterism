...
本ドキュメントは asterism の **Step 0 = ontology / MIE / ingester の設計** を、AI agent (LLM) と人間が **対話的・視覚的に協働**して作るためのワークフローを retrospective に整理したものです。
...
│      - ingest/src/asterism/{name}.py + tests/                  │
└────────────────┬─────────────────────────────────────────────┘
                 │
                 ▼
┌──────────────────────────────────────────────────────────────┐
...
```
sd:   https://kumagallium.github.io/asterism/{dataset}/ontology#
sdr:  https://kumagallium.github.io/asterism/{dataset}/resource/
...
| `ingest/src/asterism/{name}.py` | CSV → Turtle 変換、utf-8-sig open、複合 IRI、PROV-O、JSON 列展開、error log |
...
1. **MCP tool 化**: `propose_schema(csv_path, domain_hint)` / `refine_schema(comment)` を asterism に組み込み、Claude Code / Dify から AI session 内で呼べる
...
これは starrydata 以外の任意 dataset (Materials Project / NIMS Supercon / 一般 ELN data) を asterism に取り込む **障壁を下げる**ことに直結する。
...
- [`../../ingest/src/asterism/starrydata.py`](../../ingest/src/asterism/starrydata.py) — 同上
