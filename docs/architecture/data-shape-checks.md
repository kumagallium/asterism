...
**無い**場合は該当エンティティを走査しきる。ingest / append の直後に一度だけ
走る設計なので現状は許容し、規模が問題になったら内側を `LIMIT` 付きの部分
集合に切る（「最初の N 件を見た限り」に弱める）余地を残す。クエリ本数は
`max_queries`（既定 400）で上限を切っており、実マッピングの実測は 12–80 本。
