...
罠: 定数は **必ず新名前空間 `rmlf:constant`**（= `http://w3id.org/rml/constant`）。レガシー `rml:`（`http://semweb.mmlab.be/ns/rml#`）には `constant` が無く、Morph-KGC が黙って無視する。プリミティブの定数パラメータ IRI は `fn:p_table` / `fn:p_pattern` / `fn:p_template` / `fn:p_field1`…`fn:p_field4`。`regex_extract` は **re2 互換**パターンのみ（後方参照・先読み不可）。利用可能な seed 表は `ingest/src/asterism/tables/`（`bool` / `country_iso3166` / `unit_alias`）。定数は関数 IRI ではないので T9 閉集合チェックには影響しない（`rmlf:function` のみ照合）。多値の定数パラメータ IRI は `fn:p_index`（`array_at`）/ `fn:p_delimiter`（`split`）/ `fn:p_field`（`json_pluck`）。**`split` / `json_array` / `json_pluck` は `list[str]` を返し Morph-KGC が各要素を 1 トリプルへ explode**（多値の宣言的展開・入れ子 TriplesMap 不要）＝ `str -> str` 不変の例外。**罠: 多値関数は「値なし」を空 list でなく `None` で返す**（空 list は explode で NaN→直列化クラッシュ）。形ごとの選択: 1 要素配列→`json_array_single`／定位置→`array_at`／flat 区切り→`split`／JSON 文字列スカラ配列→`json_array`／JSON 文字列 object 配列の sub-field→`json_pluck`。**native JSON ソースの入れ子配列は関数に渡せない**（Morph-KGC `.str` クラッシュ・多重参照非対応・入れ子 iterator は親キー参照不可）＝子エンティティは `rml:iterator "$[*].child[*]"` の副 TriplesMap で作れるが work へのリンクは substrate 制約（denormalize 等の別 WS）。それ以外の真に不可なものだけ `…Raw`。
