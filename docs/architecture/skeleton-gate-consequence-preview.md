...
## C7-C11 改訂 — `scope-missing` も機械が押す（2026-08-27）

C10 は「`measurement-id` は機械が自動で差し替える／`scope-missing` だけは人が押す」
だった。理由は「親のキーを足すかどうかは、このデータセットが今後どう育つかの見立て
であって、機械が推測してよいことではない」。

**この線引きは間違っていた。**

- 「育つ」かどうかは画面の前提そのものである。**「データを追加」という導線が、
  ファイルが増えることを意味している。**
- 直しは推測ではない。**一意なキーに列を足しても一意のまま**（親の値が空の行は
  そもそも対象から外れる）。壊れる可能性が構造的に無い。
- 人に委ねた結果、**誰も押さなかった実行ではリスクがそのまま残った**。しかも
  「押す価値のある選択肢」と「押さないと危ないもの」が同じチップとして並んでいて、
  読み手には見分けが付かない。

改訂: `apply_key_safety_fix` は `key_measurement_caution` **または** `scope-missing`
で発火する。逃げ道は据え置き（`applied_key_fix` が差し替えを表示し、「戻す」も出る）。

実データ（XRD 参考カード・4 マップ）:

```
前: peak = xrdr:peak/{CSD}/{2theta}      線 1 本・かたまり 3 個
後: peak = xrdr:peak/{No}/{CSD}/{2theta} 線 3 本・かたまり 1 個   （2 周目は無変更＝冪等）
```

## つながりは「線」ではなく「辺」— `ensure_same_source_links`（2026-08-27）

**キーの入れ子は線であって、リンクではない。** ゲートの見取り図は「A の ID が B の
キーを内側に含む」ときに線を引くが、RDF の辺は**プロパティが書かれて初めて生まれる**。
図はつながって見えるのに、グラフはつながっていない、が成立する。

per-map 段のプロンプトには ENTITY LINKING があるが、それは**お願い**で、弱いモデルは
黙って飛ばす。島のまま compile も validate も publish も通り、
`_connectivity_advisories`（ingest 側）が拾うのは**人が全部のゲートを通した後**。

`assemble_mapping_ir` の直後に決定論の保証を置く。同一 source の map が 2 つ以上の
かたまりに割れているとき、証明できる 2 通りだけで辺を足す:

| | 根拠 | 書くもの |
|---|---|---|
| 1 | A の subject プレースホルダが B のそれを真に含む＝ A は必ずどれか 1 つの B の中 | 子の側に `dcterms:isPartOf` → 親の subject |
| 2 | B のキーが**1 列**で、その列を別の map A が**値として持っている**＝ A の 1 行は B を 1 つだけ名指す | A に `{ontology}:{bName}` → B の subject |

**2 はリテラル行を消さない。**書き換えではなく追加なので、人が確認した値が消えることは
起きない（G6 の「借り列はリンクの中だけ」とも矛盾しない — 借り列の判定は per-map 段で
既に済んでいる）。

どちらも成り立たないとき（キーも列も共有しない singleton どうし）は**黙る**。関係の
発明は機械の仕事ではなく、advisory が人に渡す。source をまたぐ辺は決して足さない
（`propose.py` の「ソースが本当に無関係なら別のかたまりでよい」に従う）。
