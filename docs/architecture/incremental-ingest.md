...
**✅ 配線完了**: 参加レジストリを YAML config 化（`asterism.crosswalk.load_crosswalk_config`・既定 `<registry_root>/crosswalk.yaml`・rule の `dataset` は canonical graph IRI の部分文字列）＋api が hub を再ビルド＝`POST /api/crosswalk/rebuild`（手動）＋**追記ごとの debounce 自動再ビルド**（`_crosswalk_rebuild_loop`：追記が `crosswalk_dirty` をセット→静穏窓後に1回再ビルド）。再ビルドは各 rule の観測を live canonical graph から読み→`build_turtle`→hub graph を **PUT で置換**（`OxigraphClient.put_turtle_bytes`・stale link 消滅）→promoted フラグ。実 Oxigraph+実 Morph-KGC で実証（`experiments/crosswalk-hub/verify_rebuild.py`＝追記で共有組成が 1→2 に育つ）。残＝promote 時にも再ビルド／観測読みの 2-pass スケール境界／per-link provenance。
