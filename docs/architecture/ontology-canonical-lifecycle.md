...
status: **ドラフト（要ユーザー確定）** — 本書は「どう作るか」を固定する提案。`要決定` 印の項目はユーザー（信頼アンカー）の確定待ち。確定後 status を「合意済み」に上げる。
...
**要決定（ユーザー確定待ち）**:
1. §2: TBox を canonical に混ぜる vs 別 `graph/ontology/{name}`（推奨=別）。
2. §3: 版の粒度（推奨=IRI 不変＋dataset version）／retract は物理削除 vs tombstone（推奨=tombstone）。
3. §4: starrydata 降格をどこまで今やるか（推奨=P2 まで先行）。
4. §5: typed ツール一般化 (a)content vs (b)生成（推奨=a 主・b 補助）。
