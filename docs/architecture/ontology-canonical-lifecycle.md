...
5. **ontology graph 投影**＋`schema_summary` の TBox 補強。✅ **完了（additive）**: 新 `asterism.ontology_projection`＝rdf-config `model.yaml` を RDFS/OWL に投影（class→`rdfs:Class`＋`rdfs:label`／predicate→`rdf:Property`＋`rdfs:label`／**単一 domain のみ `rdfs:domain`**＝多 domain は RDFS 交差の誤りを避け省略／単一 class-ref のみ `rdfs:range`）。prefix は bundle 自身の RML/MIE `@prefix`/`PREFIX`（`extract_prefixes`）∪ 標準 well-known map で解決（`sd:`/`sdr:` は dataset 固有）・未解決 term は graceful skip。promote 時に bundle の TBox を `…/graph/ontology/{id}` へ投影（DROP→load で再昇格冪等・best-effort＝失敗は log のみで promote 不阻害）。`schema_summary`(graph=None) が ontology graph 群（`substrate.ontology_graphs`）から `rdfs:label` を取得し class/predicate/class_shapes に `label` 付与。**不変条件維持**: ontology graph 不在なら label 無しで ABox 逆算のまま動く（実 rdflib＋実 Oxigraph read-only で確認）。promote API レスポンスに `ontology_graph`/`ontology_triples`。
...
- ✅ **P4-2a MCP 動的公開 着地**: `build_server` が `load_all_query_tools()`（全 dataset の `query_tools.yaml`）を読み、各宣言ツールを **typed FastMCP tool として動的登録**。schema は宣言パラメータから **`__signature__` 合成**（`inspect.Signature` + `Annotated[T, Field(description)]`・enum→`Literal`）で derive＝agent には `property_ranking(property_y, top_n, max_plausible)` の形で見える（generic dict でない）。**生成コードなし**（handler は QueryTool を捕捉した固定 closure・`__signature__` を差すだけ）。**hardcoded `sample_search`/`property_ranking` の登録を撤去**＝content が MCP surface の正（Python 関数は equivalence test 用に tools.py に残置）。名前衝突は `{dataset}_` prefix。`provenance_of`/`template_curve_fetch`/`schema_summary`/`sparql_query` は hardcoded 登録のまま。実 FastMCP 3.4.0 で `list_tools` schema・`call_tool` round-trip・required 検証・衝突 prefix をテスト。ingest 143/mcp 41 緑。
- ⬜ **P4-2b 残**: demo-agent の `_route`/`_typed_answer` を **dataset-driven** 化（現状は hardcoded `property_ranking`/`sample_search` 関数を keyword routing で直呼び＝starrydata 固定）。方針 = LLM escape ループに宣言 typed tools を MCP tool として供給し LLM が typed を優先選択（keyword routing より自然な汎用化）。② 表示ツール（provenance/curve）の宣言化・QUDT 表 per-dataset 化（§4）も後続。
