...
- P2: §4-(i) starrydata 定数/seed を `datasets/starrydata/` へ、core を汎用化（後方互換）。
  ...
  - ⬜ **P2-2b 以降（残）**: api `Settings`/mcp typed ツールが `asterism.starrydata` の定数 import をやめ descriptor/設定経由に（typed ツールは starrydata 専用なので P4 の per-dataset 化と合流）。seed（`demo-agent/seed/`）・`qudt_map.yaml`/`qudt.py`・`watcher.py` 既定 path を `datasets/starrydata/` へ物理移動。
