...
| プリミティブ | 定数引数 | 役割 |
|---|---|---|
| `lookup` | `table`（seed 表名） | 値を語彙表で引く（`bool` / `country_iso3166` / `unit_alias`、`ingest/src/asterism/tables/`・パッケージ同梱）。**キーのミス → ""**（行の事実）。**表そのものが読めない（未知の表名・不正名・ファイル欠落・YAML 不正）は "" でなく `LookupTableUnavailableError`**。表名は定数なので、設計時に `rml_validate` が did-you-mean 付きで弾く |
| `regex_extract` | `pattern`（re2 互換） | 部分文字列抽出（named group `v` → group 1 → 全体）。**google-re2 で線形時間＝ReDoS 免疫**・入力長上限。ミス → ""。**エンジン（google-re2）不在は "" でなく `RegexEngineUnavailableError`**（行の事実と環境の欠陥を混同しない） |
...
| `split` | `delimiter` | flat 区切り多値 → `list[str]`（explode で複数トリプル） |
| `json_array` | — | JSON **文字列**スカラ配列（`'["P1","P2"]'`）→ `list[str]`（explode） |
| `json_pluck` | `field` | JSON **文字列** object 配列（`'[{"family":"Adams"},…]'`）の各 `field` → `list[str]`（explode。starrydata の `author` 等） |

多値関数（`split`/`json_array`/`json_pluck`）は **`list[str]` を返し Morph-KGC が explode**（宣言的多値経路・入れ子 TriplesMap 不要・実機確認済）。**「値なし」は `None` を返す**（Morph-KGC が explode 前に行を落とす。空 `list` は explode で NaN 化し直列化を壊すため不可＝罠）。

**substrate 制約（残課題・スパイクで確定）**: object 配列の完全展開は文字列セル（CSV 等）なら `json_pluck` で可だが、**native JSON ソースの入れ子配列**は Morph-KGC が ① 多重参照 `author[*].family` 非対応 ② 関数に配列を渡すと `.str` でクラッシュ ③ 入れ子 iterator `$[*].author[*]` は子エンティティを作れるが**親キー参照不可＝work にリンクできない**。よって native JSON の入れ子 object 配列で「リンク済みエンティティ」を作るには **JSON 事前展開（denormalize）等の substrate 側設計**が要る（別ワークストリーム）。
...
- 2026-06-10: object 配列の多値展開を 2 関数で（§4・Tier0=30）。`json_array(value)`（JSON 文字列スカラ配列 → list）・`json_pluck(value, field)`（JSON 文字列 object 配列の各 `field` → list）。いずれも explode で複数トリプル。**「本丸」のスパイク結論**: Morph-KGC は ① 多重参照 `author[*].family`=0 件 ② 関数に native JSON 配列を渡すと `.str` クラッシュ ③ 入れ子 iterator `$[*].author[*]` は子エンティティ可だが親キー参照不可。よって **文字列セル（CSV 等）の object 配列は `json_pluck` で展開・親にリンク可（starrydata の `author`/`project_names` が本命）**、**native JSON ソースの入れ子は substrate 側 denormalize 設計が要る別 WS**。**罠（merged `split` も該当の潜在バグを本 PR で修正）**: 多値関数は「値なし」を空 `list` でなく **`None`** で返す（空 list は explode で NaN→直列化クラッシュ。`None` は explode 前に行 drop）。`FunctionSpec.func` 型を `str | list[str] | None` に拡張。空配列行込みの e2e（`test_materialize_json_array_and_pluck_from_string_cells`）で確認。
- 2026-08-20: `regex_extract` の **エンジン不在を "" から例外へ**（`RegexEngineUnavailableError`）。`substrate` extra を部分的にしか入れていない環境（morph-kgc あり・google-re2 なし）で、materialize が **成功のまま 0 トリプル**を返し、その列の値が警告ログ 1 行だけ残して丸ごと消えていた（実測: 修正前は `materialize_to_graph` / `materialize_to_nt_file` とも 0 triples・exit 0）。"" は **その行の事実**（該当なし）、エンジン不在は **環境の欠陥**であり、両者を同じ "" に潰したことが silent drop の原因。ミス／空入力／長さ超過／re2 非対応パターンは従来どおり ""（行の事実）とし、不在のみ loud にした＝`asterism.substrate` の morph-kgc 不在契約（`RuntimeError`・部分 materialize を作らない）と揃う。あわせて `test_materialize_with_parameterized_primitives` のスキップ条件を morph-kgc **と** google-re2 の両方に（片方だけ入った venv では「lookup と template は出るのに regex_extract だけ何も出ない」という不可解な失敗になっていた）。

- 2026-08-20: `lookup` も同じクラスの欠陥だったので閉じた（`regex_extract` の直後に発見）。**読めない表を空 dict に潰していた**ため、表名のタイポ（`booleans` / `bool`）やパッケージング漏れで **全行が "" → 列が丸ごと消えたまま materialize が成功**していた（実測: `bool.yaml` を外すと 0 triples・exit 0・警告ログ 1 行のみ）。キーのミスは **行の事実**なので "" のまま、表が読めないのは **環境／設計の欠陥**として `LookupTableUnavailableError` に。両者を貫く概念に `PrimitiveUnavailableError` という名前を与え、`RegexEngineUnavailableError` もその下に置いた（「この行に答えが無い」と「どの行にも答えられない」を型で分ける）。あわせて **設計時ゲート**を追加: `rml_validate._check_lookup_tables` が `fn:lookup` の `p_table` 定数を同梱表の閉集合と突き合わせ、`Did you mean: bool?` / `Available tables: …` を返す（Morph-KGC に届く前に落ちる＝実行時例外は defense in depth）。**未解決の周辺事実**: ingest の CI は `uv pip install -e '.[dev]'` で `substrate` extra を入れていないため、Morph-KGC 実機 e2e（この 2 件の証明を含む）が **CI では全部 skip**されている。workflow 変更は別 PR に切る必要があるので本 PR には含めない。
