...
| 20 | **オントロジー/canonical ライフサイクル + starrydata 脱結合**（層整理・CRUD/版・core から starrydata を example へ降格・typed ツール一般化） | 🟡 **ADR 合意済**（決定 4 件確定）。**P2 着手・P2-1 完了**: 汎用ヘルパを `asterism.text` に抽出＝Tier0 core が starrydata module 非依存に（後方互換 re-export、ingest 94/mcp 29/demo 9 緑）。発見=P2 は「移動」でなく untangling。残=P2-2（`DEFAULT_ONTOLOGY` 既定を core から外す・seed/QUDT を `datasets/starrydata/` へ） | core 設計→CC | `architecture/ontology-canonical-lifecycle.md` |
