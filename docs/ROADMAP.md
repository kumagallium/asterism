...
| 18 | **汎用クエリ/Ask 層**（最小=SPARQL tool → NL→SPARQL、スキーマ非依存） | 🟢 **土台＋escape＋UX 完了**。土台(LLM-free)=MCP `schema_summary`＋`sparql_query`＋demo-agent `/demo/schema`・`/demo/sparql`。escape=demo-agent `/demo/ask` を**型付き優先＋自動フォールバック**化（型ツールが引用ゼロ→LLM が schema_summary 接地で read-only SPARQL→`sparql_query` 実行→接地回答＋引用＋使用SPARQL 開示）。UX=Ask-view に一般質問用キー欄（sessionStorage 共通）＋「使用した SPARQL」開示パネル（mock で実機確認済）。残=**実 LLM dogfood**（実 Oxigraph＋新スキーマ＋実キーで一周） | core 設計→CC | — |
...
2. **#18 汎用クエリ層**: ~~土台（schema_summary + sparql_query, LLM-free）~~ ✅ ＋ ~~LLM NL→SPARQL escape（型付き優先＋自動フォールバック）~~ ✅ ＋ ~~Ask-view UX（一般質問のキー欄・SPARQL 開示パネル）~~ ✅。次=**実 LLM dogfood**（実 Oxigraph＋新スキーマ＋実キーで escape を一周し、生成 SPARQL の安定性を確認）。
