...
| — | **UI プロダクト品質化（forest 再設計）** | 🟢 **Phase 1 merged (PR #96) + Phase 2 着地（PR 進行中）**。P1=基盤(forest トークン/Fonts/border-box)+新IA骨格+Ask 全面刷新(来歴常設右パネル)+アクティビティ。P2=**ホーム**(統計+2アクション+最近のDS)+**カタログ データセット主役化**(一覧+設計図/取り込みルール タブ・実 materialized draft と promote 人間ゲートを温存)+**共有の語彙**画面(束縛戦略4種)+**データを追加 ソース切替**(CSV 実動/JSON・API・DB 近日)。**契約不変・表示のみ**。残=非CSVソースの実接続(#19)・dead CSS 整理 | CC(UI) | `design/asterism-ux/` |
