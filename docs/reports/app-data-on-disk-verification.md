...
### 配布版（v0.17.3）— 残っていた最後の経路

開発ビルドでは上書きの保存までしか確認できていなかった「**再起動して新しい場所が
実際に使われる**」を、リリースした配布版でユーザーが実機確認した（2026-08-19）。
設定 → ストレージ → 変更… → 再起動 → 新しい場所が使われる、まで一周している。

これで当初の問い（1〜4）はすべて実機で裏づけられた。
...
- Windows / Linux のデータホーム変更は未確認（macOS のみ）。
- 保存先を変えたあとのデータ移行は手動コピーのまま（導線なし）。
