...
| 2026-09-07 | [既存データセットの説明を三つ組へ移す — 実データ 7 件を実ストア込みで一周](dataset-description-migration.md) | ✅ 往復 7/7 等価・2 回目は全件 unchanged・`schema_summary` の件数不変・togomcp 配信は normalize 等価（差分は集合の並びとキー順のみ）。コメント喪失 1 件は原本に残る |
