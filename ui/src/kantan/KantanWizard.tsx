...
export function KantanWizard({
  ...
  const metaQueueRef = useRef<
    { p: RuleProperty; column: string; field: 'label' | 'unit'; value: string; before: string }[]
  >([])
  ...
  function commitMeta(p: RuleProperty, column: string, field: 'label' | 'unit', raw: string) {
    if (!kzDatasetId) return
    const value = raw.trim()
    const before = (field === 'label' ? readMeaning(p) : (p.unit ?? '')).trim()
    if (value === before) return
    patchRuleMeta(p, column, field, value)
    metaQueueRef.current.push({ p, column, field, value, before })
    ...
    try {
      while (metaQueueRef.current.length > 0) {
        ...
        for (const it of batch) {
          const key = `${it.p.predicate_iri || it.p.predicate}\u001f${it.column}`
          const row =
            byRow.get(key) ??
            ({
                              ...
                              {missing &&
                                (invented ? (
                                  ...
                                  <button
                                    ...
                                    disabled={!kzDatasetId}
                                    onClick={() => commitMeta(p, column, 'label', column)}
                                  >
