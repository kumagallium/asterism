...
def render_mermaid_body(graph: MermaidGraph) -> str:
    ...
    Shared renderer: the TBox path (this module) and the Mapping-IR path
    (:mod:`asterism_step0.ir2mermaid`) both build a :class:`MermaidGraph`
    and render through here, so the two diagrams stay byte-comparable.
    """
    buf = io.StringIO()
    ...
    for c in graph.classes:
        ...
        for prop, rng in c.datatype_properties:
            ...
            buf.write(f"        +{prop} {rng}".rstrip() + "\n")
        buf.write("    }\n")
    if graph.classes:
        buf.write("\n")
    for r in graph.relations:
        buf.write(f"    {r.domain_label} --> {r.range_label} : {r.property_label}\n")
    if graph.relations:
        buf.write("\n")
    for c in graph.classes:
        if c.subclass_of:
            buf.write(f'    note for {c.label} "subClassOf {", ".join(c.subclass_of)}"\n')
    return buf.getvalue()


def render_mermaid_block(graph: MermaidGraph) -> str:
    """Just the ```mermaid``` fenced block (no surrounding doc)."""
    return "```mermaid\n" + render_mermaid_body(graph) + "```\n"
...
def render_doc(graph: MermaidGraph, *, title: str | None = None) -> str:
    ...
    if title:
        buf.write(f"# {title}\n\n")
    buf.write(
        "Generated by `asterism-ttl2mermaid` — do not hand-edit. Re-run after TBox changes.\n\n"
    )
...
def _main(argv: list[str] | None = None) -> int:
    ...
    if args.check is not None:
        ...
        sys.stderr.write(
            ...
            "Re-run asterism-ttl2mermaid <ttl> --output <md> to regenerate.\n"
        )
        return 1
